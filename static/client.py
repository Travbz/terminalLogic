...
class engine:
    ...
    def __init__(self, api_key="apikey", api_secret="secret", subaccount_name="terminalLogic") -> None:
    ...
    def get_all_markets(self, market: str, start_time: float = None, end_time: float = None) -> List:
        limit = 100
        results = []
        start_time = "1559881511"
        resolution_ = "60" # options are 15(15second), 60(seconds), 300, 900, 3600, 14400, 86400, or any multiple of 86400 up to 30*86400
        while True:       
              response = self._get(f'markets/{market}/candles?resolution={resolution_}', {
                  'end_time': end_time,
                  'start_time': start_time,
              })
              historical_data = [r for r in response if r['startTime']not in start_time]
              results.extend(historical_data)
              print(f'Adding {len(response)} rows with end time {end_time}')
    ...
    def getData(self):
        self = requests.get('https://ftx.com/api/markets/BTC/USD/candles?resolution=60').json()
        self = pd.DataFrame(self['result'])
        self.drop(['startTime'], axis = 1, inplace=True)
